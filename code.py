...
def clear(df,col,val):
    ...
    print(f'\nTotal count of {col} is {value_counts}\n')
...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    """
    #weather[(weather['Wind Spd (km/h)'] > 35) & (weather['Visibility (km)'] == 25)]
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    #print(weather[['Date/Time','Month']])
    aggregated_value = df.pivot_table(index = 'Month', values = agg_col , aggfunc = agg)
    return aggregated_value
    #print(weather.pivot_table(index = 'Date/Time',values = 'Temp (C)', aggfunc = 'mean'))
    """
...
print("\nFetch the data set from the path:\n")
...
print("\nTo identify the categorical and numerical columns from the data frame...\n")
categorical(weather)
print(" ")
...
# x = weather[weather['Weather'] == 'Cloudy']
#print(len(x))
...
#print("\nWind speed above 35 and visibility equal to 25:\n", wind_speed_35_vis_25)
print(" ")
print("*" * 100)
...
print("To calculate the aggregate columns using Pivot Table:")
monthly_pivot_table = agg_values_ina_month (weather, 'Date/Time', 'Dew Point Temp (C)', 'mean')
...
#print(weather.groupby(['Weather']).mean())
print("To calculate the aggregate columns using GroupBy:")
...
Celisius2Fahrenheit = convert(weather, 'Temp (C)')
print(Celisius2Fahrenheit)
print(" ")
print("*" * 100)
